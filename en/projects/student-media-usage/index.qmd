...
An initial inspection for missing values was conducted using `sum(is.na(df))` , and no explicit missing data were identified in the dataset (shown in [Output 2](#num-of-NA)).
...
[*Output 2. Number of NA Values*]{.caption}
:::

## Categorical Data

For categorical variables, their numbers of categories (see [Output 3](#cat-unique)) and counts of observations in each category (see [Output 4](#cat-counts)) were first examined. The results indicated that most categorical variables contained a moderate number of categories, whereas *country* and *most-used platform* variables had relatively larger numbers of categories and therefore required further processing.
...
[*Output 3. Numbers of Unique Values in Categorical Variables*]{.caption}
:::
...
[*Output 4. Numbers of Observations of Each Value in Categorical Variables*]{.caption}
:::

Top five countries and platforms with the most counts of observations were thereby selected with their names stored ( vectors `top5_countries` and `top5_platforms` ). The results were displayed in [Output 5](#top-cats).
...
[*Output 5. Numbers of Observations in Each Country and Platform*]{.caption}
:::
...
In addition, the `Affects_Academic_Performance` variable was converted from character to logical ( "Yes" = `TRUE` , "No" = `FALSE` ) as shown in [Output 6](#bool-var).
...
[*Output 6. Boolean Variable*]{.caption}
:::

## Numerical Data

Descriptive statistics for the numerical variables were obtained using the `summary()` function in [Output 7](#num-summary). The results indicate that the variables fall within reasonable ranges, with no apparent extreme or abnormal values. Thus, no further preprocessing was deemed necessary at this stage.
...
[*Output 7. Descriptive Summaries of Numerical Variables*]{.caption}
:::

Moreover, the `Age` variable was categorized into three groups due to its limited range and variability. As shown in [Output 8](#age-range), treating `Age` as continuous offers minimal dispersion within such a narrow interval (18-24) and limited continuity (only 7 unique numbers), while treating the values as single-year categories would possibly lead to several small and uneven subgroups. Categorizing into three groups therefore provides a more suitable structure for subsequent analysis.

To achieve this, a histogram was firstly used to view age distribution and obtain suitable breaks for grouping. Then, grouping information was stored into a new variable named `Age_Group` (see [Output 9](#age-groups)).
...
[*Output 8. Range of Age and Numbers of Unique Age*]{.caption}
:::
...
[*Output 10. Percentages of Top 5 Countries Relative to the Population*]{.caption}
:::
...
To validate the trend, a Pearson's correlation test was conducted. As shown in the results (see [Output 11](#sleep-cor)), the correlation coefficient was around -0.79 (*p* \< .001). It therefore indicated that there was a high potential of students' average daily social media usage hours being negatively correlated with their sleep hours.
...
[*Output 11. Pearson's Correlation Test Result*]{.caption}
:::
...
In addition, it was also considered worthwhile investigating the relationship between other indices such as mental health score and addicted score (names of indices were shown in [Output 12](#index-names)). Therefore, a faceted 5×5 paired scatter plot matrix was used to display all of the relationship trends (see @fig-indices-scatter).
...
[*Output 12. Names of Indices to Compare*]{.caption}
:::
...
[*Output 14. Pairs of Indices with the Strongest Correlation (\|r\|\>.9)*]{.caption}
:::
...
In terms of the average media usage hours variable, a 2×2 histogram matrix (@fig-usage-sample-means-grid) of sample mean distributions with different sample sizes (n = 10, 30, 50, 100) was plotted with axis scales being the same to see the trend of distribution shape. Meanwhile, [Output 15](#usage-stats) showed the population and sample statistics regarding usage hours.
...
[*Output 15. Comparison of Statistics Regarding Media Usage Hours*]{.caption}
:::
...
Likewise, similar approaches were applied to another numerical variable sleep hours. The result patterns shown in @fig-sleep-sample-means-grid, [Output 15](#usage-stats), and [Output 16](#sleep-stats) were closely similar to those of media usage hours, again demonstrating the applicability of CLT.
...
[*Output 16. Comparison of Statistics Regarding Sleep Hours*]{.caption}
:::
...
[Output 17](#srswr), [Output 18](#ups), and [Output 19](#strata) showed the results of different samples, regarding the proportion of different platforms, and the mean and standard deviation of media usage hours.
...
[*Output 17. Results of Simple Random Sampling with Replacement*]{.caption}
:::
...
[*Output 18. Results of Systematic Sampling with Unequal Probabilities*]{.caption}
:::
...
[*Output 19. Stratified Sampling without Replacement*]{.caption}
:::
...
[Output 20](#platform-pct) presented the results of proportions of platforms in the population (top 5 platform data) and three samples. To facilitate the investigation, the means of difference between each sample value with its associated population value were calculated and shown in [Output 21](#platform-diff). Furthermore, @fig-sampling-diff visualized the results in [Output 21](#platform-diff).
...
[*Output 21. Differences from Population Percentage by Sampling Method*]{.caption}
:::
...
In addition to predicting the proportions of categories, the ability to predict mean and standard deviation across the three sampling methods were also tested. [Output 22](#usage-sampling-stats) showed the results of means and standard deviations, and a clearer visualization was shown in @fig-sampling-means-sds.
...
[*Output 22. Population and Sample Statistics of Media Usage Hours*]{.caption}
:::
...
Combining [Output 22](#usage-sampling-stats) and @fig-sampling-means-sds, it could be seen that for mean of numerical variable, stratified sampling without replacement showed the nearest result and the highest predictive power, whereas systematic sampling with unequal probabilities had the lowest predictive power.
