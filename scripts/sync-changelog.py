#!/usr/bin/env python3
"""
Sync change log data from Notion databases to _timeline.md files

1. Read Notion db: Quarto Change Log & Quarto Dev Track
2. One entry per timeline node (Type == Phase | Entry)
3. Detail bullets written in each entry's page content;
    - zh bullets as sub bullets of en ones; if no zh, fallback to en
    - Date prefix: "MMM D · [content]"
    - Type prefix: "<feat/fix/proj,etc.> [content]"
4. Generate <lang>/changes/_timeline.md files
5. Aggregate & render to timeline & heatmap by src/filters/changelog-timeline.lua
...
# Notion Quarto Change Log db prop names (only props that may be changed)
props = dict(
    start = "Start",       # no need for NY time handle
    label = "Date label",  # no need for NY time handle
    hours = "Hours",
    days = "Days",
)
...
def prop_rollup(page, name):
    rollup = _prop(page, name).get("rollup")
    if rollup:
        # rollup date
        if rollup.get("date"):
            return rollup.get("date").get("start")
        elif rollup.get("number"):
            return rollup.get("number")
        else: 
            return "rollup type undefined"
    else: 
        return None
...
def sort_key(page):
    start = prop_rollup(page, props["start"]) or "9999-12-31"
...
def fetch_timeline(token, rows):
    ...
    for i, page in enumerate(sorted_rows, start=1):
        ...
        if ptype == "Phase":
            date_range = prop_formula(page, props["label"]) or ""
            if not date_range:
                blank_formula += 1
            nodes.append({
                "type": "Phase",
            ...
            iso = prop_rollup(page, props["start"]) or ""
            if not iso:
                blank_formula += 1
            nodes.append({
                "type": "Entry",
                "name": name,
                "name_zh": name_zh,
                "date_label": prop_formula(page, props["label"]) or "",
                "iso": iso,
                "commits": fmt_num(prop_number(page, "Commits")),
                "hours": fmt_num(prop_rollup(page, props["hours"])),
                "days": fmt_num(prop_formula(page, props["days"])),
                "summary": summary,
    ...
    if blank_formula:
        warn(
            f"{blank_formula}/{total} rows have an empty {props["start"]}/"
            f"{props["label"]} — these are formula properties fed by the "
            "_start/_end/_hours/_days rollups over the 'All Projects' and "
            "'Quarto Dev Track' relations. If they look populated in Notion but "
            "arrive blank here, either the two NY sibling formulas are missing "
            "(add them — see scripts/sync-changelog.py header), or the "
            "integration isn't shared with those source databases (the API "
            "returns a relation it can't read as empty; connect it via each "
            "database → ••• → Connections). Then re-run."
        )
