...
@sec-db introduces my database structure and workflow, while @sec-tips shares key lessones I've learned from building this auto-sync workflow.


# Database Structure {#sec-db}

I'm using two project-specific database for the development track and change log of this Quarto website, as well as one general database tracking all projects.
...
The Dev Track database contains store single daily tasks as an entry. It has a Relation property connecting to the Change Log database, as well as basic properties such as timeline and hours.
...
It's worth noting that I don't manually add each row through Notion's UI, instead, I use Notion Calendar to record tasks in a clear calendar view, I can take notes while doing tasks.
...
The Change Log database records phased progress of the development of my website. Each entry in this database are related to several tasks in Dev Track database, and aggregates them into Rollup properties such as Start (actual start date: days with start time before 5AM are counted into the previous day) and Hours (the total time span of all tasks during certain period).
