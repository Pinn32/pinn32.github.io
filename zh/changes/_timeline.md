...
### 评论组件自动切换语言；搭建外部内容 {date="Jul 2" iso="2026-07-02T11:30:00.000-04:00" commits="6" hours="10" days="1"}

Giscus 评论现在会跟随页面语言切换，标签可再次点击取消高亮，外部链接加上了图标，搭建UI设计项目demo。
...
## 阶段 7 · 自动化更新日志、UI项目与博客 {range="Jul 3 - Jul 9"}
...
- contnt: 关于页面（目前为 contact）；完善 change-log；首页；优化流量歌曲数分项目中文标题
- style: 新增 about-jolla.css；优化 `cl-item:hover` 边框颜色；移除带有 `.btn` 的外部链接 `::after` 图标
- fix: 重构 `repo-url` 及链接至仓库 CSV 的逻辑
- fix: 通过页内脚本实现 404 页面的 Giscus 语言切换
- feat: 初始化 Bayanihan 项目；新增交互式 color-token-bar
- proj: 将 Bayanihan 项目翻译为中文
...
- feat: 页面导航 (页面底部)
- style: 侧边栏分隔线 (暂时禁用)；修复暗色模式下的代码工具样式
- dev: 为博客恢复按日期排序；为所有页面文件夹名称添加日期并添加别名
